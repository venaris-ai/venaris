// src/app/orga/members/page.tsx #22
...
function formatDeliveryState(invite: InviteRow, language: AppLanguage) {
  if (invite.email_sent_at) return language === "en" ? "Sent" : "Versendet";
  if (invite.email_error?.trim()) return language === "en" ? "Failed" : "Fehlgeschlagen";
  return language === "en" ? "Not sent" : "Nicht versendet";
}

function t(language: AppLanguage) {
  return language === "en"
    ? {
        missingTargetUser: "Missing target user.",
        invalidTargetRole: "Invalid target role.",
        invalidTargetStatus: "Invalid target status.",
        invalidTargetLanguage: "Invalid target language.",
        selfChangeBlocked: "You cannot change your own account here.",
        targetMemberNotFound: "Target member not found.",
        adminOwnerChangeBlocked: "Admins may not change owners.",
        adminPromoteOwnerBlocked: "Admins may not promote anyone to owner.",
        lastOwnerChangeBlocked: "The last active owner cannot be changed.",
        ownRemoveBlocked: "You cannot remove your own account here.",
        adminOwnerRemoveBlocked: "Admins may not remove owners.",
        lastOwnerRemoveBlocked: "The last active owner cannot be removed.",
        resendMissingInviteId: "[resendInvite:input] Missing invite id.",
        resendInviteNotFound: "[resendInvite:load_invite] Invite not found.",
        missingInviteId: "Missing invite id.",
        eyebrow: "Members",
        title: "Members",
        intro:
          "Manage users of the active organization here. Roles determine administrative and operational permissions within the current tenant context.",
        demoReadOnly: "Demo mode: changes are disabled.",
        invited: "Invitation was created successfully.",
        resent: "Invitation was sent again.",
        revoked: "Invitation was revoked.",
        changed: "Member changes were saved.",
        removed: "Member was removed permanently.",
        ownerStatText: "Full access to the organization.",
        adminStatText: "Operational administration within the organization.",
        memberStatText: "Regular productive users.",
        viewerStatText: "Read-only access without admin functions.",
        memberListTitle: "Member list",
        memberListText: "Current memberships of the active organization.",
        inviteMember: "Invite member",
        noMembersTitle: "No members yet",
        noMembersText: "No memberships have been created for the active organization yet.",
        emailCol: "Email",
        roleCol: "Role",
        statusCol: "Status",
        ...
        addedAtCol: "Added at",
        actionsCol: "Actions",
        openInvitesTitle: "Open invitations",
        openInvitesText: "Invitations of the active organization that have not yet been accepted.",
        noOpenInvitesTitle: "No open invitations",
        noOpenInvitesText: "As soon as you invite a new member, the invitation appears here.",
        deliveryCol: "Delivery",
        invitedAtCol: "Invited at",
        expiresAtCol: "Expires at",
        resend: "Resend",
        revoke: "Revoke",
      }
    : {
        missingTargetUser: "Zielnutzer fehlt.",
        invalidTargetRole: "Ungültige Rolle.",
        invalidTargetStatus: "Ungültiger Status.",
        invalidTargetLanguage: "Ungültige Sprache.",
        selfChangeBlocked: "Das eigene Konto kann hier nicht geändert werden.",
        targetMemberNotFound: "Mitglied nicht gefunden.",
        adminOwnerChangeBlocked: "Admins dürfen Owner nicht ändern.",
        adminPromoteOwnerBlocked: "Admins dürfen niemanden zum Owner machen.",
        lastOwnerChangeBlocked: "Der letzte aktive Owner kann nicht geändert werden.",
        ownRemoveBlocked: "Das eigene Konto kann hier nicht entfernt werden.",
        adminOwnerRemoveBlocked: "Admins dürfen Owner nicht entfernen.",
        lastOwnerRemoveBlocked: "Der letzte aktive Owner kann nicht entfernt werden.",
        resendMissingInviteId: "[resendInvite:input] Fehlende Einladungs-ID.",
        resendInviteNotFound: "[resendInvite:load_invite] Einladung nicht gefunden.",
        missingInviteId: "Fehlende Einladungs-ID.",
        eyebrow: "Mitglieder",
        title: "Mitglieder",
        intro:
          "Verwalte hier die Nutzer der aktiven Organisation. Rollen bestimmen, welche administrativen und operativen Rechte innerhalb des aktuellen Tenant-Kontexts bestehen.",
        demoReadOnly: "Demo-Modus: Änderungen sind deaktiviert.",
        invited: "Einladung wurde erfolgreich angelegt.",
        resent: "Einladung wurde erneut versendet.",
        revoked: "Einladung wurde widerrufen.",
        changed: "Mitglied-Änderungen wurden gespeichert.",
        removed: "Mitglied wurde dauerhaft entfernt.",
        ownerStatText: "Voller Zugriff auf die Organisation.",
        adminStatText: "Operative Verwaltung innerhalb der Orga.",
        memberStatText: "Reguläre produktive Nutzer.",
        viewerStatText: "Lesender Zugriff ohne Admin-Funktion.",
        memberListTitle: "Mitgliederliste",
        memberListText: "Aktuelle Memberships der aktiven Organisation.",
        inviteMember: "Mitglied einladen",
        noMembersTitle: "Noch keine Mitglieder vorhanden",
        noMembersText: "Für die aktive Organisation wurden bisher noch keine Mitglieder angelegt.",
        emailCol: "E-Mail",
        roleCol: "Rolle",
        statusCol: "Status",
        languageCol: "Sprache",
        lastLoginCol: "Letzter Login",
        addedAtCol: "Hinzugefügt am",
        actionsCol: "Aktionen",
        openInvitesTitle: "Offene Einladungen",
        openInvitesText: "Noch nicht angenommene Einladungen der aktiven Organisation.",
        noOpenInvitesTitle: "Keine offenen Einladungen",
        noOpenInvitesText: "Sobald Du ein neues Mitglied einlädst, erscheint die Einladung hier.",
        deliveryCol: "Versand",
        invitedAtCol: "Eingeladen am",
        expiresAtCol: "Läuft ab am",
        resend: "Neu senden",
        revoke: "Widerrufen",
      };
}
...
async function saveMemberChanges(formData: FormData) {
  ...
  if (!targetUserId) {
    throw new Error(text.missingTargetUser);
  }

  if (!["owner", "admin", "member", "viewer"].includes(nextRoleRaw)) {
    throw new Error(text.invalidTargetRole);
  }
  ...
  if (!["de", "en"].includes(nextLanguageRaw)) {
    throw new Error(text.invalidTargetLanguage);
  }
  ...
  if (actorUserId === targetUserId) {
    throw new Error(text.selfChangeBlocked);
  }
  ...
  if (!targetMember) {
    throw new Error(text.targetMemberNotFound);
  }

  if (actorRole === "admin" && targetMember.role === "owner") {
    throw new Error(text.adminOwnerChangeBlocked);
  }

  if (actorRole === "admin" && nextRoleRaw === "owner") {
    throw new Error(text.adminPromoteOwnerBlocked);
  }
  ...
  if (
    ...
    if (ownerCount <= 1) {
      throw new Error(text.lastOwnerChangeBlocked);
    }
  }
...
async function removeMember(formData: FormData) {
  ...
  if (actorUserId === targetUserId) {
    throw new Error(text.ownRemoveBlocked);
  }
  ...
  if (actorRole === "admin" && targetMember.role === "owner") {
    throw new Error(text.adminOwnerRemoveBlocked);
  }
  ...
  if (targetMember.role === "owner" && targetMember.status === "active") {
    ...
    if (ownerCount <= 1) {
      throw new Error(text.lastOwnerRemoveBlocked);
    }
  }
...
async function resendInvite(formData: FormData) {
  ...
  if (!inviteId) {
    throw new Error(text.resendMissingInviteId);
  }
  ...
  if (!invite) {
    throw new Error(text.resendInviteNotFound);
  }
...
async function revokeInvite(formData: FormData) {
  ...
  if (!inviteId) {
    throw new Error(text.missingInviteId);
  }
    ...
    <main className="space-y-8">
      ...
      {invited ? (
        <section className="rounded-[24px] border border-emerald-300/20 bg-emerald-300/10 p-4">
          <p className="text-sm text-emerald-100">{text.invited}</p>
        </section>
      ) : null}

      {resent ? (
        <section className="rounded-[24px] border border-emerald-300/20 bg-emerald-300/10 p-4">
          <p className="text-sm text-emerald-100">{text.resent}</p>
        </section>
      ) : null}

      {revoked ? (
        <section className="rounded-[24px] border border-emerald-300/20 bg-emerald-300/10 p-4">
          <p className="text-sm text-emerald-100">{text.revoked}</p>
        </section>
      ) : null}

      {changed ? (
        <section className="rounded-[24px] border border-emerald-300/20 bg-emerald-300/10 p-4">
          <p className="text-sm text-emerald-100">{text.changed}</p>
        </section>
      ) : null}

      {removed ? (
        <section className="rounded-[24px] border border-emerald-300/20 bg-emerald-300/10 p-4">
          <p className="text-sm text-emerald-100">{text.removed}</p>
        </section>
      ) : null}

      <section className="grid gap-4 md:grid-cols-4">
        <StatCard title="Owner" value={ownerCount} text={text.ownerStatText} />
        <StatCard title="Admin" value={adminCount} text={text.adminStatText} />
        <StatCard title="Member" value={memberCount} text={text.memberStatText} />
        <StatCard title="Viewer" value={viewerCount} text={text.viewerStatText} />
      </section>

      <section className="rounded-[28px] border border-white/10 bg-white/5 backdrop-blur-sm">
        <div className="flex items-center justify-between border-b border-white/8 px-6 py-4">
          <div>
            <h2 className="text-lg font-medium text-white">{text.memberListTitle}</h2>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-white/5 text-left text-white/55">
                <tr>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.emailCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.roleCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.statusCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.languageCol}</th>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-white/68 whitespace-nowrap">
                        {formatDateTime(authUser?.last_sign_in_at ?? null, language)}
                      </td>
      ...
      <section className="rounded-[28px] border border-white/10 bg-white/5 backdrop-blur-sm">
        <div className="border-b border-white/8 px-6 py-4">
          <h2 className="text-lg font-medium text-white">{text.openInvitesTitle}</h2>
          <p className="mt-1 text-sm text-white/65">
            {text.openInvitesText}
          </p>
        </div>
        ...
        {invites.length === 0 ? (
          <div className="px-6 py-10">
            <div className="rounded-[24px] border border-dashed border-white/10 bg-white/5 p-8">
              <h3 className="text-base font-medium text-white">
                {text.noOpenInvitesTitle}
              </h3>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-white/68">
                {text.noOpenInvitesText}
              </p>
            </div>
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm">
              <thead className="bg-white/5 text-left text-white/55">
                <tr>
                  ...
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.roleCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.statusCol}</th>
                  <th className="px-6 py-3 font-medium">{text.deliveryCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.invitedAtCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.expiresAtCol}</th>
                  <th className="px-6 py-3 font-medium whitespace-nowrap">{text.actionsCol}</th>
                </tr>
              </thead>
              <tbody>
                {invites.map((invite) => (
                  <tr key={invite.id} className="border-t border-white/8 align-middle">
                    ...
                    <td className="px-6 py-4 text-white/68">
                      <div>{formatDeliveryState(invite, language)}</div>
